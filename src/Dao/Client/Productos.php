...
class Productos extends \Dao\Table
{
    ...
    {
        return self::obtenerRegistros("SELECT * FROM agrofertil.productos p
        INNER JOIN media m on p.ProductoId = m.ProductoId WHERE ProductoStock > 0 AND ProductoEst = 'ACT'
    ...
    {
        $sqlstr = "SELECT * FROM agrofertil.productos p
        INNER JOIN media m on p.ProductoId = m.ProductoId WHERE ProductoStock > 0
    ...
    {
        $sqlstr = "SELECT * FROM agrofertil.productos p
        INNER JOIN media m on p.ProductoId = m.ProductoId
